...
事实上，多层全连接的连接处理方式很好，使得全连接神经网路具备一定非线性映射学习能力，能胜任一些简单、低维度的学习任务；但是，全连接神经网路是不适合做图像识别/分类任务的，为啥呢？先买个关子，第二章会为你解答。      
      
对于MNIST数据集，它的维度是28 x 28 x 1=784, 相对较小，对MLP来说在可接受的范围，而且MNIST数据集较为简单，所以我们用全连接神经网络也是可以实现较好的分类的。

为了使模型能够学习非线性模式（或者说具有更高的复杂度），激活函数）被引入其中。常用的激活函数有Sigmoid、tanh、Relu，如下图所示：
